...
#### Email verification code (optional but recommended)

Strava sometimes (and, for automated logins, often) challenges the login with a 6-digit one-time code emailed to `STRAVA_EMAIL` instead of, or in addition to, a password prompt. To let the app read that code automatically instead of getting stuck, add a **Gmail App Password** to `.env`:

1. Enable 2-Step Verification on the Google account, then generate an App Password at [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords).
2. Base64-encode it and add it to `.env`:
   ```bash
   printf '%s' 'your-16-char-app-password' | base64   # -> GMAIL_APP_PASSWORD
   ```
   ```dotenv
   GMAIL_APP_PASSWORD=eW91ci1hcHAtcGFzc3dvcmQ=
   ```

The app connects to `imap.gmail.com` over IMAP, waits for Strava's "Your one-time code" email (polling for up to 90 seconds, since delivery isn't instant), and submits the code automatically. If Strava shows the code screen and `GMAIL_APP_PASSWORD` isn't set, login fails with a clear error telling you to add it.
...
| `--headed` | Run the login browser with a visible window (overrides `config.json`'s `headless`) | `node dist/main.js --headed --dry-run` |
...
│   ├── emailOtp.ts           # Reads Strava's emailed one-time login code over IMAP
...
- **`src/emailOtp.ts`**: Polls Gmail over IMAP for Strava's one-time login code email and extracts the code
...
- **`imapflow`**: IMAP client used to read the emailed one-time login code
- **`mailparser`**: Decodes the fetched email so the code can be extracted reliably
...
4. **"Strava is asking for an emailed one-time code, but no Gmail App Password is configured"**
   - Add `GMAIL_APP_PASSWORD` to `.env` as described in [Email verification code](#email-verification-code-optional-but-recommended)

5. **"Timed out ... waiting for the Strava login code email to arrive"**
   - Check that `GMAIL_APP_PASSWORD` is a valid, non-revoked Gmail App Password for `STRAVA_EMAIL`
   - Confirm IMAP access isn't blocked (Gmail's "Less secure app access" is irrelevant here since App Passwords use normal IMAP auth, but check for any account-level restrictions)
